...
# 분모에 못 들어간다.
#
# 끝도 고정한다(사용자 결정 2026-09-15). 실험은 09-15에 a로 끝났고 이후
# 이벤트는 전부 variant='a'다 — 창을 열어 두면 b가 0으로 수렴하면서
# 표가 "b 0명"으로 무너지고 z가 뜻을 잃는다. 결론은
# docs/metrics/AB-TOPBAR-20260915.md. 다음 실험은 새 이름·새 창을 쓴다.
AB_START = "2026-08-20"
AB_END = "2026-09-15"
AB_WINDOW = ("timestamp >= toDateTime('%s 00:00:00') AND timestamp <= toDateTime('%s 23:59:59')"
             % (AB_START, AB_END))
...
"""),
    ...
    # ---- 5. A/B ---------------------------------------------------------
    {
        ...
        "description":
            "**종료(09-15, a 확정). 창 08-20~09-15 고정.** 결론: docs/metrics/AB-TOPBAR-20260915.md\n\n"
...
"""),
    ...
    # ---- A/B 행동 지표 (기존 A/B 대시보드에 붙는다) ----------------------
    {
        ...
        "description":
            ...
            "a는 조건을 바에 펼치고 b는 시트에 감춰 **갈래마다 나오는 이벤트가 "
            "다르다**(a엔 `filter_sheet_open`이 없다). 이 표의 '설정률'은 "
