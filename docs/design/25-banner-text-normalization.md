...
결정: 당일 행사 배너(`banners.yml`)의 자유 문장 두 칸(`period`, `extra`)을 오퍼 구간과 같은 필드(금액, 최소주문, 열림 시각, 선착순 여부, 채널, 멤버십)로 바꾸고, 화면 문구는 필드에서 만든다. 지금은 계획만 세운다. 도입 시점은 22번 문서(검수 화면)와 같이 정한다. 검수 화면의 배너 입력 폼이 자유 문장 두 칸이 아니라 이 필드들이 되어야 하기 때문이다.
...
1. 묶음 배너의 표현. `amounts`와 `minOrders` 병렬 배열인가, `items: [{brand, amount, minOrder}]` 객체 배열인가. 추천은 객체 배열이다. 브랜드별 열림 시각(`12시-노랑통닭 · 17시-BBQ`)까지 담으려면 결국 객체가 필요하다. 이 경우 `brands`와 `amount`는 `items`에서 파생한다.
2. 랜덤 쿠폰의 값 표기. `amountRange` 칸을 따로 두는가, `amount: [3000, 8000]`으로 재활용하는가. 추천은 별도 칸이다. `amount`가 정수가 아니면 웹의 폭 계산(`amountIsWide`)이 갈린다.
3. 이행 시점. 22번 검수 화면과 같이 하는가(추천), 1단계와 2단계만 먼저 하는가. 지금 해서 얻는 것은 매일 나는 헛 경고 제거뿐이라 22번과 묶는 쪽이 싸다.
