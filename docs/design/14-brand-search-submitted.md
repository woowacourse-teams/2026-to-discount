...
사용자가 브랜드 검색을 확정하는 순간 `brand_search_submitted` 이벤트를 App의 공통 검색 제출 경계에서 한 번 발행하기로 정했다(B안). 검색어 원문은 싣지 않고 길이, 결과 수, 제출 방식만 기록한다. GitHub 이슈 #14(`[WEB][API] feat: 브랜드 검색 제출 이벤트 계측 추가`)의 설계다. 구현이 끝나 이슈는 닫혔다.
...
- 검색어가 비어 있음: 검색 상태만 갱신하고 이벤트를 보내지 않는다.
- 브랜드 데이터 미로딩: 제출 이벤트는 보내되 `resultCount`를 생략한다.
