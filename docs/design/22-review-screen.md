# 22. 검수 화면: 웹에서 오퍼와 배너를 고치고 운영에 내보내는 화면 (계획)

자동 수집이 dev에 올린 결과를 사람이 실제 화면에서 고치고 "반영" 한 번으로 운영에 내보내는 검수 화면을 제안한다. 이 문서는 계획까지이고 도입 시점은 정하지 않았다. 착수는 별도 지시로 한다(추적 이슈 #28). 전제는 21번(개발/운영 분리)이 먼저라는 것이다. 이 화면은 **dev 웹과 dev API에서만** 켜진다.
...
1. 21번 1~3(dev API, dev 웹, 미러). 전제다.
