...
1. 반영 주체 (가) 미니PC / (나) 서버 — 위 §5, 추천 (나).
2. 검수 없이 지나간 수집분은 언제 운영에 가나 — 21번 §5-4의 (가) 시한 자동 /
   (나) 검수 전엔 안 감. 이 화면이 생기면 (나)가 자연스럽지만 사람이 매일 봐야
   한다는 뜻이다. 주말·부재 시 규칙 필요.
3. "빼기"의 원장 표현 — `expires_at = 오늘`(ADR-028 "오늘 만료는 관측이 아니다"와
   충돌 가능) vs 새 필드 `withdrawn: true`. 스키마 한 칸 추가가 더 정직하다 —
   그 경우 ADR 하나.
4. 편집 검증을 dev 서버에서 파이썬으로 하나, 자바로 옮기나, 미니PC ingest에
   맡기나(추천).
