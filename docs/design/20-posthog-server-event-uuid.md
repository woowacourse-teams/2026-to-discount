...
서버 릴레이 이벤트에도 브라우저가 만든 `eventId`를 최상위 `uuid`로 실어, PostHog SDK 경로와 서버 릴레이 경로의 같은 이벤트가 같은 식별자를 쓰게 하기로 정했다. `PostHogEvent`에 `uuid` 필드를 추가하는 A안을 골랐다. 이 문서는 GitHub 이슈 #20의 설계이고 구현이 끝나 이슈는 닫혔다.
