...
2. 현재 `notify: true`이고 발송되지 않은 activation을 찾는다. 정상 운영에서는 직전 24시간 대상이며, 스케줄 누락이나 장기 중단 뒤에는 그보다 오래된 미발송 대상도 복구한다.
...
- 서버 재시작 → 영속 상태를 읽어 처리 중이거나 미발송인 작업을 재개한다. Push 서비스가 성공을 반환한 직후 성공 이력을 저장하기 전에 종료되면 같은 요청을 재전송할 수 있으므로 전달 보장은 at-least-once다. 재전송은 같은 `notificationId`와 알림 `tag`를 사용해 기존 알림을 대체하고 분석 이벤트 ID도 동일하게 유지한다.
