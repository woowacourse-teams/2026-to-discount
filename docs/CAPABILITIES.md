...
무엇이 실제로 돌고 있는지를 한자리에 모은다. **이 문서는 목록이고, 왜
그렇게 골랐는지는 각 문서에 있다.**
...
견주는** 페이지다. 로그인이 없고, 사용자는 카드를 보고 앱 딥링크로 나간다.
...
나머지 셋은 사람이 스크린샷을 찍어 넘기기로 했다(tracker ADR-013). 그 뒤 앱들이
화면을 고치면서 목록에 텍스트가 생겨 그 전제가 깨졌다.

| 앱 | 읽는 화면 | 걸리는 점 |
|---|---|---|
| 배달의민족 | 브랜드관 딥링크, `배짱할인 > 오늘의 핫딜` | 없음 |
...
브랜드`, 요기요 행사 페이지의 이미지 항목이 여기에 해당한다. `scripts/full_audit.py`가 네 앱을
한 번에 훑는다.

### 배너 루틴: 새벽에 올리고 발급 전에 확인만

핫딜은 하루 안에도 바뀐다(실측: 전날 8,000원이던 처갓집이 다음 날 7,000원).
...
검증이 안 고치는 이유는 선착순 발급이 11시이기 때문이다. 그 직전에 기계가 배너를
...
앞에 붙여 둔 경우) 지난 배너가 이긴다. `minOrder`도 물려받는다. 쿠팡이츠
배너는 상한을 못 읽으면 오늘 배너 → 지난 배너 순으로 물려받는다. 뽑고
나면 화면에서 상한이 사라지는데, 그때마다 배너가 퇴화하면 안 되기 때문이다.
...
**원장(`events.jsonl`)이 제품 이벤트 19종의 권위 있는 기록이고, PostHog은 탐색용이다.**
...
| 편향 방지 | 집계 단위가 **사람**. 한 명이 43번 눌러도 1. 중앙값과 최다 1명을 같은 표에 찍어 왜곡을 눈치챌 수 있게 |
...
- 자유 입력은 200자 상한이다. 전화, 이메일, 주민번호는 서버가 지운다.

**개발자가 배포한 화면에서 확인하려면:** `DISCOUNT_SURVEY_TEST_VISITORS`에
visitorId를 콤마로 넣으면 그 id만 접속일과 전환수 조건을 건너뛴다. 1인 1회와 재고는
그대로 걸려 실사용자와 같은 경로를 지난다. 확인이 끝나면 비운다.

이 우회를 둔 이유는 다른 길이 둘 다 되돌릴 수 없어서다.

- 실제 대상자의 visitorId를 빌리면 그 사람 몫의 기프티콘이 나가고, 원장에 그 사람 이름으로 응답이 박힌다.
- 원장에 가짜 방문을 심으면 append-only라 흔적이 영구히 남는다.
