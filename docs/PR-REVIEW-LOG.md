...
반영했는지"를 나중에 다시 grep 안 해도 되게 하는 것이다. 리뷰 자체의 회고
문서가 아니라 처리 로그다.
...
2. `git status --short`로 워킹트리가 비어 있는지 확인하고 나서만 그다음
   ...
   `git add`에 같이 잡히는 게 2026-08-14 사고의 원인이었다. 상태 확인 없이
   연달아 커밋하지 않는다.
...
(2026-08-06 기준 20건 부활)로 적혀 있었다. `is_stale_sweep` 도입
(2026-08-10)으로 실제로는 해소됐다. 문서만 안 따라간 상태를 반영했다.
...
- [x] `is_stale_sweep`가 전수 수집이 성립하는 4개 플랫폼(배민, 쿠팡이츠,
  ...
  §4, `tracker/docs/ORCHESTRATION-CONTRACT.md` §1과 §5 세 군데 다.
...
- CodeRabbit이 `a29d1f5` 재리뷰를 rate limit으로 못 돌렸다. 지적사항은
  내용상 이미 다 반영하고 검증했다고 판단해 재리뷰 없이 머지(squash,
  브랜치 삭제).
...
`--check` 모드로 CI(`check-project-structure.yml`)에서 검증한다. ADR-001(문서가
stale인 걸 아무도 몰랐던 사고)이 동기다.
...
- 엔드포인트 가드의 false-positive 위험(매핑 아닌 애노테이션도 잡을 수
  있음)
- `API_RESPONSIBILITIES` 등 설명 상수를 누가 소유하고 유지하는지
- path filter를 제거한 근거(보안 함의 있음에도)
- CI를 self-hosted 대신 `ubuntu-latest`로만 돌리는 이유. 외부 PR이
  배포 권한 있는 러너에 접근하지 못하게 하려는 의도로 보이고, 타당해 보임
...
- 엔드포인트 정규식(`@(Get|Post|Put|Delete|Patch)Mapping\("..."\)` 단일
  ...
  `TestDataController` 등)와 전수 대조. 전부 이 형태라 지금 시점
...
dev 트래픽과 visitorId 없는 이벤트는 전달 제외, ipHash 외부 미전달, 기본
비활성.
...
- [x] resend 중복제거 문서와 구현 불일치. `IncomingEvent`에 `eventId`
...
않은 채로 이어서 "PR#2 리뷰 로그" 커밋을 만들었다. 그 커밋이
...
없이 실패한 체크가 사라진 상태로 정리됐다. self-hosted 배포 워크플로
...
"반영 확인, 머지 진행"이라고 판단했다. 그 판단이 이 사고의 진짜 원인이다.
...
- §1에 "재생성 입력은 원장 한 파일이 아니라 원장 + `data/sweeps.jsonl` 두
  파일"을 명시(ADR-020).
- §5, §6 수치를 2026-08-16자로 갱신(export 160건, 종료일 없는 오퍼 110건,
  yogiyo 94%, `needs_review` 원장 43).

## tracker#1: data: 원장 363행 증거 상태 전수 확정 (리베이스 요청, 열린 채)

내용 리뷰 전에 상태를 지적했다. base `2d71c60`, 그 뒤 main이 크게 이동해 4파일 충돌
...
08-13 리뷰에서 로직과 보안 검증 후 **생성기 유닛테스트 부재**만 남겨두고 열어뒀던
PR이다. 이틀간 반영이 없었고 그 사이 main이 24커밋 움직여, 테스트를 리뷰어가 후속
...
보존하게 하는 변경이다. 서버 절반은 PR#4에서 이미 들어와 있었다
...
## 사고 기록: 2026-08-16, 미러링이 운영 코드를 옛 버전으로 덮을 뻔함

ADR-018은 `nn98/*`에서 개발해 모노레포로 미러링한다고 정한다. 그런데 팀 PR
(#4 PostHog, #6 eventId)은 **모노레포로 직접 들어온다.** 그 방향이 원본에
역이식되지 않아 원본이 운영보다 뒤처진 상태였다.

`membership_toggle` 하나를 넣으려고 원본 `EventController.java`를 모노레포로
복사했더니 컴파일이 깨졌다. 원본에는 `eventId` 필드가 없었기 때문이다.
빌드가 잡아줘서 드러났지만, 필드가 아니라 값만 다른 파일이었으면 조용히
운영 코드가 되돌아갔다.

**조치**: 모노레포 `api/src`와 `web/src/analytics.js`를 정본으로 삼아 원본에
역이식(api `0f22d09`, web `fd159ef`). 양쪽 테스트 통과.

**규칙**: 미러링 전에 `Get-FileHash`로 양방향 차이를 먼저 확인한다. 팀 PR이
모노레포로 직접 들어온 뒤에는 그 변경을 원본에 역이식하고 나서 다음 작업을
시작한다. 한쪽 방향만 흐른다고 가정하지 않는다.

## 사고 기록: 2026-08-16, 구조 검사가 하루 세 번 빨개짐

`generate_project_structure.py`가 `git ls-files`(추적 중인 파일만)로 서명을
만들었다. 새 파일을 만든 직후 로컬에서 `--check`를 돌리면 그 파일이 안
세어져 통과하는데, 커밋해서 푸시하면 CI는 세므로 실패한다.

같은 함정에 세 번 걸렸다. 브랜드 8건 등록(`brands.yml`은 기존 파일이었지만
tracker 미러에 새 파일이 있었다), 인코딩 helper, `vercel.json`.

**조치**: `--cached --others --exclude-standard`로 바꿔 아직 추가 안 된
파일도 센다(`a81b995`). 커밋하면 어차피 들어올 파일이므로 지금 같이 세는
것이 맞고, `.gitignore` 대상은 그대로 빠진다.
