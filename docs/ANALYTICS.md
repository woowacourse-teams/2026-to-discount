...
| 이벤트 | 언제 | 붙는 값 |
|---|---|---|
| `offer_link_click` | 할인 칩을 눌러 배달앱으로 나갈 때 | `brand`, `platform` + 어느 오퍼인지(09-17~): `amount`, `minOrder`, `qualifier`, `membership`, `tierMode`, `tiers`(구간 수), `best`, `held`, `soldOut`, `fromBanner`, `position`(카드 순번, 09-20~) |
| `banner_click` | 상단, 하단 행사 배너 | `banner`(id), `brand`, `platform`(배달앱 밖 자체 행사는 API가 `own`으로 내려준다, 09-18~), `position`(top/bottom) + `amount`, `brands`(묶음 `a/b/c`), `minOrder`, `soldOut`, `external`(09-17~) |
...
```bash
python scripts/experiments.py compare --by period:2026-08-19 --only returning
...
재방문     34.8%          18.0%         z=-5.28  "재방문자 반토막"
```

**틀렸다.** 그 개편이 `banner_click`을 새로 만들었다. 배너도 앱으로
나가는 외부 링크인데, 그 이벤트는 08-19 전에 구조적으로 0이다. 옛
정의로는 개편 이후가 무조건 낮게 나온다.

```
전환 = offer_link_click | banner_click
재방문     34.5%          34.7%         z=0.07  판정 불가
```

**총 유출은 안 줄었다. 오퍼 칩에서 배너로 옮겨간 것이다.**

> 전후 비교에서 지표를 고를 때 **"그 사이에 이 지표의 분자나 분모를
> 바꾼 배포가 있었나"**를 먼저 확인한다. 있으면 그 배포 이후에도 뜻이
> 같은 지표로 다시 정의한다.

`--goal`이 이벤트 하나만 받으므로, 이런 경우는 지금 손으로 합쳐 센다.
