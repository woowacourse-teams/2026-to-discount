...
| `DISCOUNT_SURVEY_TEST_VISITORS` | 접속일 7일, 전환 5회 조건과 1인 1회를 건너뛴다 | 서버 환경변수, visitorId별 | 재고(있으면 준다, 없으면 없다고 답한다. 실사용자와 같다) |
...
`dev:true`가 실린다. 서버는 이 값을 보고 다음처럼 동작한다.
...
- **우리가 이미 아는 건 안 묻는다.** 주 이용 플랫폼처럼 클릭과 링크
  이동으로 로그에 이미 잡히는 값은 설문에서 빼야 한다. 물어봐도
  중복 확인일 뿐이고, 그 자리만큼 진짜 궁금한 것을 못 묻는다.
- **직접 답해야만 아는 것만 남긴다.** 쓰는 이유와 시점, 뭘 중시하는지,
...
- **프론트만 고쳤을 때** (문구, 둘째와 셋째 섹션 선택지 추가): 모노
...
- **첫 섹션 토큰을 바꿨을 때** (서버 화이트리스트도 고침): 위와
  ...
  ssh <REMOTE> "sudo systemctl restart delivery-discount-api"
  ```
