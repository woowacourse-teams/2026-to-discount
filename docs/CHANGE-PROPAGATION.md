...
한 곳을 고쳤는데 다른 곳이 같이 틀어지는 일을 부르는 말은 층마다 다르다. 섞어 쓰면
무엇을 고쳐야 할지가 흐려진다.

| 말 | 무엇을 가리키나 | 언제 쓰나 |
| --- | --- | --- |
| 결합(coupling) | 두 자리가 같이 움직여야 하는 관계 그 자체 | 설계를 이야기할 때 |
| 커네선스(connascence) | 결합 중에서 이름, 값, 순서, 시점으로 묶인 것 | 어떤 종류의 결합인지 가릴 때 |
| 동반 변경 묶음(co-change cluster) | 실제로 늘 같이 바뀌는 파일 묶음 | 관측된 사실을 적을 때. 이 문서의 표 한 줄 |
| 논리적 결합(logical coupling) | 코드 상 의존이 없는데도 같이 바뀌는 관계 | 의존 그래프에 안 잡히는 결합을 말할 때 |
| 폭발 반경(blast radius) | 한 변경이 닿는 범위 | 작업 전에 "어디까지 번지나"를 물을 때 |
| 변경 증폭(change amplification) | 하나를 고치려고 몇 곳을 고쳐야 하는가 | 설계가 나빠지는 정도를 잴 때 |
| 변경 영향 분석(change impact analysis) | 그 범위를 미리 재는 활동 | 작업 순서를 정할 때 |
| 이음매(seam) | 묶음을 끊을 수 있는 지점 | 묶음을 없앨 방법을 찾을 때 |

이 저장소에서 자주 나오는 커네선스는 셋이다.

- **이름의 커네선스**: 같은 칸 이름이 여러 곳에 적혀 있다. `firstCome`이 수집기, 콘솔
  스키마, API, 웹 넷에 적혀 있다.
- **값의 커네선스**: 같은 숫자가 여러 곳에 적혀 있다. 즉시 알림 창 07~22시가 서버와
  수집기 둘에 적힌 적이 있다(2026-09-24에 서버 한 곳으로 되돌렸다).
- **시점의 커네선스**: 무엇을 먼저 배포하느냐가 결과를 바꾼다. API가 웹보다 먼저다.

앞의 둘은 자동 검사로 묶을 수 있다. 셋째는 사람이 순서를 지키는 수밖에 없어서 제일
자주 틀린다.

## 이 저장소에서 실제로 난 일

표를 읽기 전에 이 넷을 본다. 전부 2026-09에 났고, 전부 고친 쪽 테스트는 초록이었다.

**선착순 표식이 사라질 뻔했다.** 웹 `bannerTag`가 문장 파싱을 버리고 `firstCome` 칸만
읽게 바뀌었다. 수집기는 여전히 그 칸을 안 쓰고 문장에만 "선착순"을 적었다. 승인했으면
선착순 배너 일곱 장에 표식이 안 붙는다. 배포 전에 제안서를 눈으로 보다가 찾았다.
- 묶음: 배너 모양(2번)
- 왜 테스트가 못 잡았나: 웹 테스트는 칸이 있는 픽스처를 썼고, 수집기 테스트는 문장만 봤다. 둘을 맞대는 자리가 없었다.
- 해결: 수집기가 칸을 채운다. 값은 전날 운영자 배너에서 읽었다.

**새벽 제안 아홉 건이 통째로 반려됐다.** 이어 쓰기가 전날 배너의 `notifyImmediately`를
물려받았는데, 즉시 알림은 07~22시에만 고를 수 있다. 05:26에 승인하니 한 건이 아니라
묶음 전부가 되돌아갔다.
- 묶음: 알림(5번)
- 왜 테스트가 못 잡았나: 수집기 테스트에 시각 개념이 없었다. 검사는 서버에 있었다.
- 해결: 등록만 잇고 즉시 발송은 안 잇는다. 창 숫자는 서버 한 곳에만 둔다.

**정렬이 풀린 채 배포됐다.** 웹을 먼저 올려서 새 웹이 옛 API 응답을 읽었다. `certainty`가
안 와서 전부 확정으로 떨어졌고 특정메뉴 쿠폰이 맨 위에 섰다. 배너 표식도 사라졌다.
- 묶음: 배포 순서(8번)
- 왜 테스트가 못 잡았나: 양쪽 다 제 저장소에서는 초록이다. 옛 응답에 새 웹을 물리는 자리가 없다.
- 해결: API 먼저, 확인 뒤 웹. 문서에 순서를 못박았다.

**콘솔이 통째로 안 떴다.** `norm()`을 모듈로 빼면서 `web/ops_norm.js`를 새로 만들었다.
배포 목록에는 넣었지만 nginx `types`에 `js`가 없어 `application/octet-stream`으로 나갔고,
모듈 스크립트는 MIME 검사가 엄격해 통째로 안 돈다. 화면은 "불러오는 중"에서 멈췄다.
- 묶음: 새 파일(9번)
- 왜 테스트가 못 잡았나: 테스트는 모듈을 import 해서 돌린다. 브라우저가 그 파일을 어떻게 받는지는 안 본다.
- 해결: `types`에 `js`를 넣었다. 배포 뒤 캡처를 봐서 찾았다.

네 건의 공통점 하나. **테스트는 묶음의 반대편을 안 본다.** 그래서 이 문서가 있다.
...
적어 두는 것은 임시방편이다. 줄이는 수단은 넷이고, 위로 갈수록 좋다.

1. **없앤다.** 한쪽이 다른 쪽 결과를 그대로 받게 만든다. 웹이 서버가 계산한 정렬값을
   그대로 쓰면 확실성 묶음(1번)이 사라진다.
...
넷 다 안 되면 이 표에 적는다. 적는 것은 다섯째가 아니라 실패의 기록이다.

## 이 문서를 고칠 때

- 묶음을 하나 줄였으면 그 줄을 지운다. 줄어든 것이 이 문서의 성과다.
- 새 사고가 나면 "이 저장소에서 실제로 난 일"에 한 건 더한다. 네 칸을 다 적는다:
  묶음, 왜 테스트가 못 잡았나, 해결, 그리고 무엇이 화면에 보였나.
