...
| [의사결정기록](docs/decisions/) | 되돌리기 어려운 판단 8건과 근거 |
...
| 앱 | 대상 화면 | 화면 형태 | 저비용 교차검증 |
|---|---|---|---|
| 배달의민족 | `브랜드관` → `오늘의 할인` | 네이티브 리스트 | 가능 |
| 쿠팡이츠 | `와우컬렉션` | WebView + 이미지 혼재 | 부분 |
| 땡겨요 | `오늘 땡길만한 브랜드` | 100% 이미지 배너 | **없음** |
| 요기요 | 미조사 | — | — |
| 배달특급 | 미조사 | — | — |
...
- Android SDK 에뮬레이터 헤드리스 — BlueStacks는 리눅스 미지원이라 제외 ([ADR-002](docs/decisions/ADR-002-android-sdk-emulator.md))
- 로컬 Windows PC와 리눅스 서버 양쪽에서 동일 동작해야 함
