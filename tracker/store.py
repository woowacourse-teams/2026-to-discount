...
#
#   expires_at도 목록에 찍히는 값이라 badge와 같은 이유로 병합하지 않는다.
#   한때는 넣어 뒀는데(빼면 23건이 만료일을 잃는다는 이유였다), 그 23건은
#   "덜 잃는" 대신 **틀린 날짜를 얻고** 있었다. 2026-09-04 실측: 쿠팡이츠
#   명랑핫도그·뚜레쥬르가 그날 허브에 멀쩡히 걸려 있는데도 export에서
#   사라졌다. 전날 쿠폰함에서 읽은 "오늘까지"(발급 쿠폰의 진짜 만료일)가
#   같은 브랜드의 허브 타일 레코드로 옮겨붙어, 오늘 본 오퍼가 어제 만료된
#   것으로 판정됐다. 쿠폰은 당일 만료돼도 프로모션은 계속 도는데 둘이
#   한 (앱, 브랜드) identity로 뭉치니 생긴 일이다.
#
#   빼면 배민 20건이 표시용 만료일을 잃지만 생사에는 영향이 없다 — 배민
#   만료 판정은 전수 수집이 한다(ADR-020). 쿠팡이츠 3건은 추정이 받는다
#   (ADR-023). 추정이 관측을 이기지 않게 하려면 원장에 없는 만료일을
#   만들어내지 않는 것이 먼저다.
#
