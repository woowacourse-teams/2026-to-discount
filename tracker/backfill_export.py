...
EXCLUDED = {
    # 일반 4,000원과 배민클럽 전용 7,500원 두 쿠폰. latest_per_brand가
    # (앱, 브랜드) 하나만 남겨 캡처 시각이 같은 둘 중 금액 큰 쪽이 이긴다.
    ("baemin", "청년피자"),
    # qualifier에 "특정 메뉴 할인"이 들어 있다. ADR-004의 qualifier는
    # 금액 수식어(최대/최소 — 상한이냐 하한이냐)라 허용값이 좁고, 이 값은
    # 수식어가 아니라 적용 범위 조건이라 넓히는 것으로는 해결되지 않는다.
    # 이 레코드는 badge("배민클럽 전용쿠폰")도 따로 쓰고 있어, 화면이 라벨
    # 슬롯을 둘 쓰는 중이라는 뜻이다 — 어느 필드에 담을지가 설계 문제다.
    ("baemin", "열정국밥"),
}
