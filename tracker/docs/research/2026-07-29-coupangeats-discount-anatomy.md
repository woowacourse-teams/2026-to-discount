...
## 프로모션 기간은 배너 이미지 안에 있다 (2026-08-05 추가)

만료일(`expires_at`)을 앱별로 얼마나 얻을 수 있는지가 "종료된 프로모션을
무엇으로 무력화할 것인가"를 좌우한다. 원장 138건 기준 보유율:
...
**쿠팡이츠 0%는 "기간이 화면에 없다"는 뜻이 아니다.** 두 군데에 나온다
(사용자 실사용 확인, 2026-08-05):

- **하단 유의사항 문구** — 행사 기간이 텍스트로 적혀 있다. 접근성 트리에
  올라오므로 `dump_ui()`로 바로 읽을 수 있다. 여기가 1순위다.
- **배너 이미지** — 기간이 이미지 안에 찍혀 있다. 유의사항이 없을 때의
  대안이지만 이미지를 읽는 단계가 더 필요하다.

다만 **배너가 없는 브랜드가 일부 있다** — 전 브랜드 커버는 아니다.

땡겨요가 86%인 건 브랜드관 상세가 `2026.08.03 ~ 2026.08.31`을 눈에 띄는
자리에 텍스트로 주기 때문이고(2026-08-05 전수 순회로 확인), 쿠팡이츠와의
차이는 정보의 유무가 아니라 **어디를 봐야 하는지 몰랐다는 것**이다. 지금
0%인 건 유의사항 문구를 판독 대상으로 삼은 적이 없어서다 — 무력화 설계를
확정할 때 쿠팡이츠를 "만료일을 알 수 없는 앱"으로 분류하면 안 된다.
...
- **매장 편차**: 최소주문금액이 "매장별"이라 브랜드 단위로는 대표값밖에
  ...
  같은 취급).
- **약관·접근 제한**: 이 도메인의 `robots.txt`는 `User-Agent:* / Disallow: /`
  로 자동 접근을 전면 거부한다. 엣지 차단(`/block`)까지 있어 의사가 기술적으로도
  명시돼 있다. **이 조사에서 그 차단을 우회하지 않았고, 앞으로도 하지 않는다** —
  규칙으로 못박은 문서가 [ADR-015](../decisions/ADR-015-open-access-only-and-disclosure.md)다.
  사람이 정상 사용하며 보는 화면을 캡처하는 지금 방식은 그 선 안에 있다.
