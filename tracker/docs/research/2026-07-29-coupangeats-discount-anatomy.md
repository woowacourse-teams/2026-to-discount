...
**쿠팡이츠 0%는 "기간이 화면에 없다"는 뜻이 아니다.** 배너 이미지 안에
프로모션 기간이 찍혀 나온다(사용자 실사용 확인). 다만 접근성 트리에
텍스트로 올라오지 않아 지금의 `dump_ui()` 텍스트 판독으로는 안 잡힐 뿐,
이미지를 읽는 단계를 하나 더 두면 얻을 수 있다.

땡겨요가 86%인 건 브랜드관 상세가 `2026.08.03 ~ 2026.08.31`을 **텍스트로**
주기 때문이다(2026-08-05 전수 순회로 확인). 쿠팡이츠와의 차이는 정보의
유무가 아니라 **판독 난이도**다 — 무력화 설계를 확정할 때 쿠팡이츠를
"만료일을 알 수 없는 앱"으로 분류하면 안 된다.
