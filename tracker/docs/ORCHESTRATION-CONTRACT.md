...
전까지는 종료일 없는 오퍼를 내릴 방법이 아예 없었다. 지금 export 160건 중
110건이 종료일이 없고, 그 전부를 수집 단위 판정이 맡는다.

**입력은 원장 한 파일이 아니라 두 파일이다.** `is_stale_sweep`이 쓰는 "이번
...
- **종료된 프로모션을 원장에 적을 자리는 여전히 없다.** 다만 재현은 된다.
  종료일이 있으면 `expires_at`으로, 없으면 수집 단위로 내리기 때문이다(§1).
  남은 한계는 전수 수집이 아닌 날의 관측만으로는 종료를 판정하지 못한다는
  것이다. 명시적인 무력화 필드는 아직 없다.
- **만료일 보유율이 앱마다 다르다.** ddangyo 88%, yogiyo 94%, baemin 0%,
  coupangeats 0%(2026-08-16 기준). 만료일이 없는 110건은 전부
