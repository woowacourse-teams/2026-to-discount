...
| [ADR-020](ADR-020-sweep-is-recorded-not-inferred.md) | 전수 수집은 `sweeps.jsonl`에 기록하고, 건수로 추정하지 않는다 | 확정 |
...
| [ADR-030](ADR-030-exclusion-is-a-judgement-not-an-observation.md) | 제외는 관측이 아니라 판단이다, `excluded` 필드로 적고, 지속형 제외는 같은 쿠폰의 이후 자동 관측을 이긴다 | 확정 |
| [ADR-031](ADR-031-open-baemin-brand-pages-by-resolved-url.md) | 배민 브랜드관은 단축링크가 아니라 HTTP로 풀린 `baemin.com/bridge/…` 주소로 연다(앱이 단축링크를 홈으로 보낸다) | 확정 |
| [ADR-032](ADR-032-full-audit-once-a-day-and-dead-link-ledger.md) | 전수조사는 00:01만(월요일 세 번), 링크 오류 3회 연속은 주간 주기로만 재시도 | 확정 |
