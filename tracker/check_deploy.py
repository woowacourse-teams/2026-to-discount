...
같은 이유로, 상세 소실 검사도 "의도한 삭제"와 "사고"를 구분하지 못한다.
일부러 지운 필드에 막혔다면 우회 플래그를 찾지 말고 서버 export.json을
먼저 같은 모양으로 고친 뒤 배포를 다시 돌린다 — 절차와 그 이유는
docs/decisions/ADR-024. 플래그를 만들지 않는 이유도 거기 있다.
"""
import json
import sys
