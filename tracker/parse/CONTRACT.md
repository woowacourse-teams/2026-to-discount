...
| `badge` | 목록 액면 금액이 실제로는 못 받는 상태(재고 소진)거나 매일 바뀌는 상태(랜덤박스)면 짧은 상태 라벨("선착순 품절", "선착순" 등)을 넣는다. `conditions`처럼 문장이 아니라 칩에 그대로 얹을 한두 단어. 예(쿠팡이츠 메가MGC커피, 2026-08-03 실측): 목록엔 "최대 20,000원"인데 상세를 열어보니 20,000원 티어가 품절이라 `badge: "선착순 품절"`. |
