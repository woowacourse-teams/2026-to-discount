...
> **이 저장소에는 판독 구현이 없다.** 화면을 실제로 여닫고 접근성 트리를
> 읽는 자동화(`capture/`, ADB 조작·앱별 진입 경로·좌표 상수)는 모노레포로
> 옮기면서 제외했다. 여기 남은 것은 그 자동화가 **무엇을 지켜야 하는지**를
> 정한 계약과, 그 계약이 만든 결과물(`data/`)·검증(`tests/`)·데이터
> 모델(`schema.py`·`store.py`·`export_data.py`)이다.
>
> 이 문서는 그래서 "구현 설명서"가 아니라 **판독 규칙의 명세**로 읽어야
> 한다. 실행 코드가 필요하면 별도로 받아야 한다
