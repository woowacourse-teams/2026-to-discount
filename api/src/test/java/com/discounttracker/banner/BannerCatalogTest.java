...
class BannerCatalogTest {
    ...
    void theOldShapeAndTheNewShapeReadTheSameOnScreen() {
        ...
        // 기간 문구는 두 경로가 다르다. 사람은 "9/24 하루"로 적어 왔고 서버는
        // "9월 24일 하루"를 만든다. 서버 형식이 기준이다(2026-09-24 개발자 확정) -
        // 구조를 고친 목적이 자동화라 사람이 적는 쪽을 기준으로 삼으면 자동으로
        // 만든 배너마다 손을 대야 한다. 손으로 적은 옛 배너는 그 문장이 그대로
        // 남는다(RULES 9: 지난 배너는 변환하지 않는다).
