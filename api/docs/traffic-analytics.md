...
예전에는 서버가 PostHog로 넘길 때 `device == "desktop" AND 뷰포트 폭 < 400px`
이면 `dev_suspect: true`를 붙였다. **이 규칙이 안드로이드 폰 사용자 368명을
개발자로 몰아냈다.** `device`는 프론트가 `matchMedia('(hover: hover)')`로
정하는데, 일부 안드로이드 브라우저가 `hover:hover`를 보고한다. 걸러낸 무리의
전환율이 남긴 쪽보다 높았던 것(41.9% 대 33.9%)이 단서였다.
...
이벤트 하나만 보는 서버는 내릴 수 없는 판단이다. 그래서 여기서는 판정하지
않고 `viewport`를 그대로 실어 보내기만 한다. 판정은 원장 전체를 읽는
`scripts/experiments.py` 한 곳에만 있다.

원장이 원본을 그대로 갖고 있어서 규칙이 틀린 것을 알았을 때 과거 데이터를
다시 셀 수 있었다. PostHog는 보낸 시점의 속성이 박혀 있어 그럴 수 없다 —
**수집 시점에 판단을 굳히지 않는다**는 방침이 실제로 값을 했다.
