...
chips = f"""
<ul class="offer-grid">
  <li class="offer offer--confirmed offer--best offer--hero"><button type="button" class="offer__chip"><span class="offer__amount"><span class="offer__best-label" aria-label="최고 할인"><span>최고</span><span>할인</span></span>10,000원</span>{icon('baemin','배달의민족')}</button></li>
...
<body><div class="kit">
<h1>오늘의할인 UI 키트</h1>
<p>운영 화면에서 그대로 떠낸 마크업과 CSS(2026-09-16, mono <code>main</code>(운영)). 피그마에서 <b>html.to.design</b>으로 이 주소를 가져오면 편집 가능한 레이어가 된다. 고친 프레임은 Figma MCP로 읽어 CSS로 역반영한다.</p>
...
<h2>브랜드 카드 (360px)</h2>
<div class="kit__row">
  <div class="kit__frame kit__frame--mobile"><p class="kit__cap">.brand-card 펼침 — 최고 할인 라벨, 오퍼 그리드, 구간 4칸 격자</p>{cards[0]}</div>
...
<h2>오퍼 칩 상태</h2>
<div class="kit__row"><div class="kit__frame kit__frame--mobile"><p class="kit__cap">.offer--best (최고 할인 라벨) · 멤버십 배지 · .offer--capped (상한, 점선) · .offer--held (보류) · 품절</p>{chips}</div></div>
