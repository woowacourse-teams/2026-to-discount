...
배포든 항상 그 주소로 API를 호출한다. 별도 환경변수나 프록시 설정이 필요
...
배포는 Vercel(`beggars-five.vercel.app`)이며, 백엔드 CORS 허용 목록에
이미 등록돼 있다.
...
- `src/ga4.js`: GA4 임시 도입(`startGa4()`). 이유와 제거 조건은
...
펼치는 방법은 둘이다. 카드 헤더 클릭, 금액 칩 클릭(바로가기 링크가 있는 칩은
...
2. **`src/posthog.js`(PostHog SDK)**: 도메인 `track()` 이벤트와 `page_exit`를
   ...
   `$pageleave`로 자동 수집하고, Web Vitals와 표준 기기와 브라우저 속성도 SDK가
...
4. **`src/ga4.js`(GA4, 임시)**: 위 방식만으로는 재방문을 정확히 못 재서
   ...
   전달된다. 광고 개인화, Google Signals는 꺼서 붙였다. 도입 배경과 제거
...
`page_view`를 찍고, 탭 가시성 변화와 `pagehide`에 체류 시간 전송을 건다.
...
2. 발생 지점에서 `track('새이벤트', { ...props })`를 호출한다. `props`는
   문자열 값의 얕은 객체만이다. 서버가 문자열 120자, 6개 초과분은 잘라낸다.
